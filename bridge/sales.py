...
def get_sales_voucher_details(vcode):
    ...
    sql_items = f"""
        SELECT t2.SrNo, t2.MasterCode1 AS ItemCode, t2.D1 AS Qty, t2.D2 AS Price,
               m.Name AS ItemName, m.CM1 AS UnitCodeStr, m.NameSL AS ItemNameSL
        FROM Tran2 t2
        LEFT JOIN Master1 m ON t2.MasterCode1 = m.Code
    ...
    while not rst_i.EOF:
        ...
        unit_code_str = str(rst_i.Fields("UnitCodeStr").Value or "").strip()
        ...
        unit_name = "PCS"
        unit_name_hi = ""
        if unit_code_str and unit_code_str.isdigit():
            try:
                ucode = int(unit_code_str)
                rst_u = _get_rs(f"SELECT Name, NameSL FROM Master1 WHERE Code = {ucode}")
                if not rst_u.EOF:
                    unit_name = str(rst_u.Fields("Name").Value or "").strip()
                    unit_name_hi = str(rst_u.Fields("NameSL").Value or "").strip()
                rst_u.Close()
            except Exception:
                pass
